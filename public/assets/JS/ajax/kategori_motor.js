...
$(document).ready(function()
{
	...
	{	
		...
		if( $("#idk").val() == ''){  // <---- cek id kategorinyo ado dak, men dak katek dy add.  id ini hidden di view
			var link="kategoriMotorAdd";
		}
		else if( $("#idk").val() !== ''){
			var link= "kategoriMotorEdit";
		}
	...
	function loaddata(){ // <--- ambil data
		...
		$.ajax({
			url : "kategoriMotor",  // <--- link akses ke controller
			...
			success:function(data, textStatus, jqXHR) 
						...
						//	console.log(data);  <--- ini untuk cout ke console di google chrome/firefox
						},
						error: function(jqXHR, textStatus, errorThrown) 
						{
	...
	function update(e){
		if(confirm('Did u really wanna update this?')){
			...
			$.ajax(
					{
						url : 'kategoriMotorSearch',  // <--- nak nyari data sesuai id itu
						type: "POST",
						data : {id:e.id},
						success:function(data, textStatus, jqXHR) 
						{
